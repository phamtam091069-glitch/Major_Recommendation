...
class ChatbotContextTests(unittest.TestCase):
    ...
    def test_off_topic_romance_sentence_with_noise_should_be_scope_guarded(self):
        chatbot = make_chatbot()

        result = chatbot.chat("Tôi thích linh mục tiêu sẽ cua được Linh", history=[])

        self.assertEqual(result["source"], "scope_guard")
        self.assertIn("không phù hợp", result["reply"])

    def test_off_topic_no_accent_romance_should_be_scope_guarded(self):
        chatbot = make_chatbot()

        result = chatbot.chat("toi thich Linh", history=[])

        self.assertEqual(result["source"], "scope_guard")
        self.assertIn("tư vấn ngành học", result["reply"])

    def test_phrase_thich_hop_should_not_be_misclassified_as_romance(self):
        chatbot = make_chatbot()

        result = chatbot.chat(
            "Tôi khá hướng nội, có thích hợp không?",
            history=[{"role": "user", "content": "Em muốn tìm hiểu ngành Marketing"}],
            active_major="Marketing",
        )
        ...
        self.assertEqual(result["resolved_topic"], "fit_factors")
        self.assertEqual(result["resolved_major"], "Marketing")
        self.assertNotEqual(result["source"], "scope_guard")
        self.assertIn("hướng nội", result["reply"])
        self.assertIn("vẫn có thể hợp", result["reply"])

    def test_personality_fit_for_marine_major_should_answer_with_trait_aware_guidance(self):
        chatbot = make_chatbot()
        ...
        self.assertEqual(result["source"], "context_followup")
        self.assertEqual(result["resolved_topic"], "fit_factors")
        self.assertIn("vẫn có thể theo ngành", result["reply"])
        self.assertIn("cân nhắc kỹ", result["reply"])

    def test_health_followup_keeps_existing_major_context(self):
        chatbot = make_chatbot()
    ...
    def test_hot_temper_followup_keeps_existing_major_context(self):
        chatbot = make_chatbot()

        result = chatbot.chat(
            "Tính nóng thì có học lái tàu được không",
            history=[{"role": "user", "content": "Em muốn tìm hiểu ngành Điều khiển và quản lý tàu biển"}],
            active_major="Dieu khien va quan ly tau bien",
            active_topic="fit_factors",
        )
        ...
        self.assertEqual(result["resolved_major"], "Dieu khien va quan ly tau bien")
        self.assertEqual(result["resolved_topic"], "fit_factors")
        self.assertIn("tính nóng", result["reply"])

    def test_hiv_followup_keeps_existing_major_context(self):
        chatbot = make_chatbot()
        ...
        self.assertEqual(result["resolved_major"], "Dieu khien va quan ly tau bien")
        self.assertEqual(result["resolved_topic"], "fit_factors")
        self.assertIn("HIV", result["reply"])

    def test_personality_fit_without_major_should_ask_to_clarify_major(self):
        chatbot = make_chatbot()

        result = chatbot.chat("Em hướng nội, có hợp không?", history=[])

        self.assertEqual(result["source"], "context_clarify")
        self.assertTrue(result["needs_clarification"])
        self.assertEqual(result["resolved_topic"], "fit_factors")
        self.assertIn("phù hợp với ngành nào", result["reply"])

    def test_score_only_message_should_ask_to_clarify_instead_of_predicting_major(self):
        chatbot = make_chatbot()

        result = chatbot.chat("Điểm thi cđ mà", history=[])

        self.assertEqual(result["source"], "context_clarify")
        self.assertTrue(result["needs_clarification"])
        self.assertEqual(result["resolved_topic"], "admission_score")
        self.assertIn("điểm thi/điểm xét tuyển", result["reply"])

    def test_score_followup_with_existing_major_should_use_context_followup(self):
        chatbot = make_chatbot()
        ...
        self.assertEqual(result["resolved_major"], "Marketing")
        self.assertEqual(result["resolved_topic"], "admission_score")
        self.assertIn("mức điểm xét tuyển", result["reply"])

    def test_vague_major_choice_question_should_ask_to_clarify_instead_of_predicting_major(self):
        chatbot = make_chatbot()

        result = chatbot.chat("Em muốn thi đâu thi học ngành nào", history=[])

        self.assertEqual(result["source"], "context_clarify")
        self.assertTrue(result["needs_clarification"])
        self.assertEqual(result["resolved_topic"], "major_selection")
        self.assertIn("nên học ngành nào", result["reply"])
    ...
    def test_off_topic_appearance_question_should_be_scope_guarded(self):
        chatbot = make_chatbot()

        result = chatbot.chat("Bạn có thấy tôi đẹp trai không", history=[])

        self.assertEqual(result["source"], "scope_guard")
        self.assertIn("ngoại hình", result["reply"])

    def test_off_topic_hair_style_request_should_be_scope_guarded(self):
        chatbot = make_chatbot()

        result = chatbot.chat("Chọn giúp tôi kiểu tóc", history=[])

        self.assertEqual(result["source"], "scope_guard")
        self.assertIn("không phù hợp", result["reply"])

    def test_greeting_should_ask_for_self_description_and_future_direction(self):
        chatbot = make_chatbot()

        result = chatbot.chat("Xin chào", history=[])

        self.assertEqual(result["source"], "greeting")
        self.assertIn("Mô tả bản thân", result["reply"])
        self.assertIn("Định hướng tương lai", result["reply"])
