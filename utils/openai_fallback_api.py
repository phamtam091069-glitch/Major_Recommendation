"""
OpenAI API Fallback Module
Handles communication with OpenAI-compatible API (chiasegpu)
"""
...
def get_openai_fallback_api():
    """Factory function to get OpenAI API instance."""
    return OpenAIFallbackAPI()


def get_last_fallback_api():
    """Factory function to get the final OpenAI-compatible fallback instance."""
    return OpenAIFallbackAPI(env_prefix="LAST_FALLBACK")


def get_bottom_fallback_api():
    """Factory function to get the bottom-most OpenAI-compatible fallback instance."""
    return OpenAIFallbackAPI(env_prefix="BOTTOM_FALLBACK")


class OpenAIFallbackAPI:
    """Handles OpenAI API calls for chatbot fallback."""
...
- Không nhắc rằng bạn đang làm theo prompt hay quy tắc nội bộ."""
    ...
    def __init__(self, env_prefix: str = "OPENAI"):
        """Initialize OpenAI API client."""
        self.env_prefix = str(env_prefix or "OPENAI").strip().upper()
        self.api_key = os.getenv(f"{self.env_prefix}_API_KEY", "")
        self.base_url = os.getenv(f"{self.env_prefix}_BASE_URL", "https://llm.chiasegpu.vn/v1")
        self.model = os.getenv(f"{self.env_prefix}_MODEL", "claude-haiku-4.5")
        ...
        try:
            ...
            logger.info("✓ %s API client initialized", self.env_prefix)
    ...
    def analyze_free_text(
        ...
        """
        Send user text to OpenAI API and get response.
        ...
        try:
            ...
            logger.info(f"✓ OpenAI API response generated (length: {len(response_text)})")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if is_working:
                ...
                logger.warning(f"✗ OpenAI API connection test: FAILED - {response.get('error', 'Unknown error')}")
